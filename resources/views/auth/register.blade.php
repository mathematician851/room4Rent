		...
		<div class="card">
			...
			<div class="card-body">
				<form method="POST" action="{{ route('register') }}" enctype="multipart/form-data">
					...
					<div class="form-group row">
						<label for="campus_id" class="col-md-4 col-form-label text-md-right">{{ __('Campus / Location') }}</label>
						...
						<div class="col-md-6">
							<select class="form-control{{ $errors->has('campus_id') ? ' is-invalid' : '' }}" id="campus_id" name="campus_id" value="{{ old('campus_id') }}"  }}> 
								...
								@foreach($campus as $cam)
									<option value="{{$cam->id}}">{{$cam->name}}  {{$cam->description}}</option>
					...
					<div class="row form-group">
						...
						<div class="col-md-6">
							<input id="passport" required type="file" class="form-control{{ $errors->has('passport') ? ' is-invalid' : '' }}" name="passport" value="{{ old('passport') }}" >
